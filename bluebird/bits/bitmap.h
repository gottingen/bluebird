...
#ifndef BLUEBIRD_BITS_BITMAP_H_
#define BLUEBIRD_BITS_BITMAP_H_
...
#include <bluebird/bits/roaring/roaring.h>

#undef ROARING_API_NOT_IN_GLOBAL_NAMESPACE

#include <bluebird/bits/roaring/roaring_array.h>  // roaring::internal array functions used

namespace bluebird {

    class BitmapSetBitForwardIterator;

    class Bitmap {
        typedef roaring::api::roaring_bitmap_t roaring_bitmap_t;  // class-local name alias
    ...
    public:
         ...
         */
        Bitmap(size_t n, const uint32_t *data) : Bitmap() {
            roaring::api::roaring_bitmap_add_many(&roaring, n, data);
        }

        /**
         * Construct a bitmap from an initializer list.
         */
        Bitmap(std::initializer_list<uint32_t> l) : Bitmap() {
         ...
         */
        Bitmap(const Bitmap &r) : Bitmap() {
            if (!roaring::api::roaring_bitmap_overwrite(&roaring, &r.roaring)) {
                ROARING_TERMINATE("failed roaring_bitmap_overwrite in constructor");
            }
            roaring::api::roaring_bitmap_set_copy_on_write(
                    &roaring,
         ...
         */
        Bitmap(Bitmap &&r) noexcept: roaring(r.roaring) {
            //
            ...
            //
            roaring::api::roaring_bitmap_init_cleared(&r.roaring);
        }
         ...
         */
        explicit Bitmap(roaring_bitmap_t *s) noexcept: roaring(*s) {
         ...
         */
        static Bitmap bitmapOf(size_t n, ...) {
            Bitmap ans;
            va_list vl;
            va_start(vl, n);
         ...
         */
        static Bitmap bitmapOfList(std::initializer_list<uint32_t> l) {
            Bitmap ans;
         ...
         * Add value x
         */
        void add(uint32_t x) noexcept { roaring::api::roaring_bitmap_add(&roaring, x); }
        ...
        bool addChecked(uint32_t x) noexcept {
            return roaring::api::roaring_bitmap_add_checked(&roaring, x);
        }
        ...
        void addRange(const uint64_t min, const uint64_t max) noexcept {
            return roaring::api::roaring_bitmap_add_range(&roaring, min, max);
        }
        ...
        void addRangeClosed(const uint32_t min, const uint32_t max) noexcept {
            return roaring::api::roaring_bitmap_add_range_closed(&roaring, min, max);
        }
        ...
        void addMany(size_t n_args, const uint32_t *vals) noexcept {
            roaring::api::roaring_bitmap_add_many(&roaring, n_args, vals);
        }

        /**
         * Remove value x
         */
        void remove(uint32_t x) noexcept { roaring::api::roaring_bitmap_remove(&roaring, x); }
        ...
        bool removeChecked(uint32_t x) noexcept {
            return roaring::api::roaring_bitmap_remove_checked(&roaring, x);
        }
        ...
        void removeRange(uint64_t min, uint64_t max) noexcept {
            return roaring::api::roaring_bitmap_remove_range(&roaring, min, max);
        }
        ...
        void removeRangeClosed(uint32_t min, uint32_t max) noexcept {
            return roaring::api::roaring_bitmap_remove_range_closed(&roaring, min, max);
        }

        /**
         * Return the largest value (if not empty)
         */
        uint32_t maximum() const noexcept { return roaring::api::roaring_bitmap_maximum(&roaring); }

        /**
         * Return the smallest value (if not empty)
         */
        uint32_t minimum() const noexcept { return roaring::api::roaring_bitmap_minimum(&roaring); }

        /**
         * Check if value x is present
         */
        bool contains(uint32_t x) const noexcept {
            return roaring::api::roaring_bitmap_contains(&roaring, x);
        }
        ...
        bool containsRange(const uint64_t x, const uint64_t y) const noexcept {
            return roaring::api::roaring_bitmap_contains_range(&roaring, x, y);
        }
        ...
        Bitmap &operator=(Bitmap &&r) noexcept {
            roaring::api::roaring_bitmap_clear(&roaring);  // free this class's allocations
         ...
         */
        Bitmap &operator=(std::initializer_list<uint32_t> l) {
         ...
         */
        Bitmap &operator&=(const Bitmap &r) noexcept {
            roaring::api::roaring_bitmap_and_inplace(&roaring, &r.roaring);
            return *this;
        }
         ...
         */
        Bitmap &operator-=(const Bitmap &r) noexcept {
            roaring::api::roaring_bitmap_andnot_inplace(&roaring, &r.roaring);
            return *this;
        }
         ...
         */
        Bitmap &operator|=(const Bitmap &r) noexcept {
            roaring::api::roaring_bitmap_or_inplace(&roaring, &r.roaring);
            return *this;
        }
         ...
         */
        Bitmap &operator^=(const Bitmap &r) noexcept {
            roaring::api::roaring_bitmap_xor_inplace(&roaring, &r.roaring);
            return *this;
        }
         ...
         */
        void swap(Bitmap &r) noexcept { std::swap(r.roaring, roaring); }

        /**
         * Get the cardinality of the bitmap (number of elements).
         */
        uint64_t cardinality() const noexcept {
            return roaring::api::roaring_bitmap_get_cardinality(&roaring);
        }

        /**
         * Returns true if the bitmap is empty (cardinality is zero).
         */
        bool isEmpty() const noexcept { return roaring::api::roaring_bitmap_is_empty(&roaring); }

        /**
         * Returns true if the bitmap is subset of the other.
         */
        bool isSubset(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_is_subset(&roaring, &r.roaring);
        }

        /**
         * Returns true if the bitmap is strict subset of the other.
         */
        bool isStrictSubset(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_is_strict_subset(&roaring, &r.roaring);
        }
        ...
        void toUint32Array(uint32_t *ans) const noexcept {
            roaring::api::roaring_bitmap_to_uint32_array(&roaring, ans);
        }
        ...
        void rangeUint32Array(uint32_t *ans, size_t offset, size_t limit) const noexcept {
            roaring::api::roaring_bitmap_range_uint32_array(&roaring, offset, limit, ans);
        }

        /**
         * Return true if the two bitmaps contain the same elements.
         */
        bool operator==(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_equals(&roaring, &r.roaring);
        }
        ...
        void flip(uint64_t range_start, uint64_t range_end) noexcept {
            roaring::api::roaring_bitmap_flip_inplace(&roaring, range_start, range_end);
        }
        ...
        void flipClosed(uint32_t range_start, uint32_t range_end) noexcept {
            roaring::api::roaring_bitmap_flip_inplace(
        ...
        bool removeRunCompression() noexcept {
            return roaring::api::roaring_bitmap_remove_run_compression(&roaring);
        }
         ...
         */
        bool runOptimize() noexcept { return roaring::api::roaring_bitmap_run_optimize(&roaring); }

        /**
         * If needed, reallocate memory to shrink the memory usage. Returns
         * the number of bytes saved.
         */
        size_t shrinkToFit() noexcept { return roaring::api::roaring_bitmap_shrink_to_fit(&roaring); }
        ...
        void iterate(roaring::api::roaring_iterator iterator, void *ptr) const {
            roaring::api::roaring_iterate(&roaring, iterator, ptr);
        }
        ...
        bool select(uint32_t rnk, uint32_t *element) const noexcept {
            return roaring::api::roaring_bitmap_select(&roaring, rnk, element);
        }

        /**
         * Computes the size of the intersection between two bitmaps.
         */
        uint64_t and_cardinality(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_and_cardinality(&roaring, &r.roaring);
        }

        /**
         * Check whether the two bitmaps intersect.
         */
        bool intersect(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_intersect(&roaring, &r.roaring);
        }
         ...
         */
        double jaccard_index(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_jaccard_index(&roaring, &r.roaring);
        }

        /**
         * Computes the size of the union between two bitmaps.
         */
        uint64_t or_cardinality(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_or_cardinality(&roaring, &r.roaring);
        }

        /**
         * Computes the size of the difference (andnot) between two bitmaps.
         */
        uint64_t andnot_cardinality(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_andnot_cardinality(&roaring, &r.roaring);
        }
         ...
         * bitmaps.
         */
        uint64_t xor_cardinality(const Bitmap &r) const noexcept {
            return roaring::api::roaring_bitmap_xor_cardinality(&roaring, &r.roaring);
        }
        ...
        uint64_t rank(uint32_t x) const noexcept {
            return roaring::api::roaring_bitmap_rank(&roaring, x);
        }
        ...
        int64_t getIndex(uint32_t x) const noexcept {
            return roaring::api::roaring_bitmap_get_index(&roaring, x);
        }
         ...
         *
         *       BOOST_SERIALIZATION_SPLIT_FREE(Bitmap)
         *       namespace boost {
         *       namespace serialization {
         *
         *       template <class Archive>
         *       void save(Archive& ar, const Bitmap& bitmask,
         ...
         *      }
         *      template <class Archive>
         *      void load(Archive& ar, Bitmap& bitmask,
         *          const unsigned int version) {
         ...
         *         ar&  boost::serialization::make_binary_object(buffer.data(),
         *            size_in_bytes);
         *         bitmask = Bitmap::readSafe(buffer.data(), size_in_bytes);
         *      }
         *      }  // namespace serialization
         *      }  // namespace boost
         */
        size_t write(char *buf, bool portable = true) const noexcept {
            if (portable) {
                return roaring::api::roaring_bitmap_portable_serialize(&roaring, buf);
            } else {
                return roaring::api::roaring_bitmap_serialize(&roaring, buf);
            }
        }
         ...
         * data does not match the portable Bitmap specification: you should
         * ensure that the data you load come from a serialized bitmap.
         */
        static Bitmap read(const char *buf, bool portable = true) {
            roaring_bitmap_t *r = portable
                                  ? roaring::api::roaring_bitmap_portable_deserialize(buf)
                                  : roaring::api::roaring_bitmap_deserialize(buf);
            if (r == NULL) {
         ...
         * ensure that the data you load come from a serialized bitmap.
         */
        static Bitmap readSafe(const char *buf, size_t maxbytes) {
            roaring_bitmap_t *r =
                    roaring::api::roaring_bitmap_portable_deserialize_safe(buf, maxbytes);
            if (r == NULL) {
                ROARING_TERMINATE("failed alloc while reading");
            }
            return Bitmap(r);
        ...
        size_t getSizeInBytes(bool portable = true) const noexcept {
            if (portable) {
                return roaring::api::roaring_bitmap_portable_size_in_bytes(&roaring);
            } else {
                return roaring::api::roaring_bitmap_size_in_bytes(&roaring);
            }
        }
         ...
         */
        static const Bitmap frozenView(const char *buf, size_t length) {
            const roaring_bitmap_t *s =
                    roaring::api::roaring_bitmap_frozen_view(buf, length);
            if (s == NULL) {
        ...
        Bitmap operator&(const Bitmap &o) const {
            roaring_bitmap_t *r = roaring::api::roaring_bitmap_and(&roaring, &o.roaring);
            if (r == NULL) {
         ...
         */
        Bitmap operator-(const Bitmap &o) const {
            roaring_bitmap_t *r = roaring::api::roaring_bitmap_andnot(&roaring, &o.roaring);
            if (r == NULL) {
         ...
         */
        Bitmap operator|(const Bitmap &o) const {
            roaring_bitmap_t *r = roaring::api::roaring_bitmap_or(&roaring, &o.roaring);
            if (r == NULL) {
         ...
         */
        Bitmap operator^(const Bitmap &o) const {
            roaring_bitmap_t *r = roaring::api::roaring_bitmap_xor(&roaring, &o.roaring);
            if (r == NULL) {
        ...
        void setCopyOnWrite(bool val) noexcept {
            roaring::api::roaring_bitmap_set_copy_on_write(&roaring, val);
        }

        /**
         * Print the content of the bitmap
         */
        void printf() const noexcept { roaring::api::roaring_bitmap_printf(&roaring); }
        ...
        bool getCopyOnWrite() const noexcept {
            return roaring::api::roaring_bitmap_get_copy_on_write(&roaring);
        }
         ...
         * This function may throw std::runtime_error.
         */
        static Bitmap fastunion(size_t n, const Bitmap **inputs) {
            ...
            roaring_bitmap_t *c_ans = roaring::api::roaring_bitmap_or_many(n, x);
            if (c_ans == NULL) {
                roaring_free(x);
                ROARING_TERMINATE("failed memory alloc in fastunion");
            }
        ...
        typedef BitmapSetBitForwardIterator const_iterator;
 ...
 */
    class BitmapSetBitForwardIterator final {
    public:
        ...
        typedef BitmapSetBitForwardIterator type_of_iterator;
        ...
        void equalorlarger(uint32_t val) {
            roaring::api::roaring_move_uint32_iterator_equalorlarger(&i, val);
        }

        type_of_iterator &operator++() {  // ++i, must returned inc. value
            roaring::api::roaring_advance_uint32_iterator(&i);
            return *this;
        }

        type_of_iterator operator++(int) {  // i++, must return orig. value
            BitmapSetBitForwardIterator orig(*this);
            roaring::api::roaring_advance_uint32_iterator(&i);
            return orig;
        }

        type_of_iterator &operator--() { // prefix --
            roaring::api::roaring_previous_uint32_iterator(&i);
            return *this;
        }
        ...
        bool operator==(const BitmapSetBitForwardIterator &o) const {
        ...
        explicit BitmapSetBitForwardIterator(const Bitmap &parent,
            ...
            if (exhausted) {
                ...
                roaring::api::roaring_init_iterator(&parent.roaring, &i);
            }
        }

        roaring::api::roaring_uint32_iterator_t i{}; // The empty constructor silences warnings from pedantic static analyzers.
    };

    inline BitmapSetBitForwardIterator Bitmap::begin() const {
        return BitmapSetBitForwardIterator(*this);
    }

    inline BitmapSetBitForwardIterator &Bitmap::end() const {
        static BitmapSetBitForwardIterator e(*this, true);
        return e;
    }

}  // namespace bluebird

#endif  // BLUEBIRD_BITS_BITMAP_H_
