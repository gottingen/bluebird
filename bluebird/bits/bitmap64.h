/**
 * A C++ header for 64-bit Bitmap Bitmaps,
 * implemented by way of a map of many
 * 32-bit Bitmap Bitmaps.
 *
...
namespace bluebird {
    ...
    using roaring::api::roaring_iterator64;
    class Bitmap64SetBitForwardIterator;

    class Bitmap64SetBitBiDirectionalIterator;
    ...
    public:
         ...
         */
        Bitmap64(size_t n, const uint32_t *data) { addMany(n, data); }

        /**
         * Construct a bitmap from a list of 64-bit integer values.
         */
        Bitmap64(size_t n, const uint64_t *data) { addMany(n, data); }

        /**
         * Construct a bitmap from an initializer list.
         */
        Bitmap64(std::initializer_list<uint64_t> l) {
         ...
         */
        explicit Bitmap64(const Bitmap &r) { emplaceOrInsert(0, r); }

        /**
         * Construct a 64-bit map from a 32-bit rvalue
         */
        explicit Bitmap64(Bitmap &&r) { emplaceOrInsert(0, std::move(r)); }
         ...
         */
        explicit Bitmap64(roaring_bitmap_t *s) {
            emplaceOrInsert(0, Bitmap(s));
        }

        Bitmap64(const Bitmap64 &r) = default;

        Bitmap64(Bitmap64 &&r) noexcept = default;

        /**
         * Copy assignment operator.
         */
        Bitmap64 &operator=(const Bitmap64 &r) = default;

        /**
         * Move assignment operator.
         */
        Bitmap64 &operator=(Bitmap64 &&r) noexcept = default;

        /**
         * Assignment from an initializer list.
         */
        Bitmap64 &operator=(std::initializer_list<uint64_t> l) {
         ...
         */
        static Bitmap64 bitmapOf(size_t n...) {
            Bitmap64 ans;
            va_list vl;
            va_start(vl, n);
         ...
         */
        static Bitmap64 bitmapOfList(std::initializer_list<uint64_t> l) {
            Bitmap64 ans;
        ...
        void addMany(size_t n_args, const uint64_t *vals) {
            ...
            Bitmap *last_inner_bitmap = nullptr;
         ...
         */
        Bitmap64 &operator&=(const Bitmap64 &other) {
            if (this == &other) {
         ...
         */
        void swap(Bitmap64 &r) { roarings.swap(r.roarings); }
        ...
        uint64_t cardinality() const {
            ...
            return std::accumulate(
                    ...
                    [](uint64_t previous,
                       const std::pair<const uint32_t, Bitmap> &map_entry) {
        ...
        bool isFull() const {
            ...
            return roarings.size() ==
                   ...
                   ? std::all_of(
                            ...
                            [](const std::pair<const uint32_t, Bitmap> &roaring_map_entry) {
         ...
         */
        bool isSubset(const Bitmap64 &r) const {
         ...
         */
        bool isStrictSubset(const Bitmap64 &r) const {
        ...
        bool removeRunCompression() {
            return std::accumulate(
                    ...
                    [](bool previous, std::pair<const uint32_t, Bitmap> &map_entry) {
         ...
         * readSafe.
         */
        static Bitmap64 read(const char *buf, bool portable = true) {
            Bitmap64 result;
            // get map size
            uint64_t map_size;
            ...
            for (uint64_t lcv = 0; lcv < map_size; lcv++) {
                ...
                buf += sizeof(uint32_t);
                // read map value Bitmap
                Bitmap read_var = Bitmap::read(buf, portable);
                // forward buffer past the last Bitmap Bitmap
         ...
         */
        static Bitmap64 readSafe(const char *buf, size_t maxbytes) {
            ...
            for (uint64_t lcv = 0; lcv < map_size; lcv++) {
                ...
                // read map value Bitmap
                Bitmap read_var = Bitmap::readSafe(buf, maxbytes);
                // forward buffer past the last Bitmap Bitmap
        ...
        size_t getSizeInBytes(bool portable = true) const {
            ...
            return std::accumulate(
                    ...
                    [=](size_t previous,
                        const std::pair<const uint32_t, Bitmap> &map_entry) {
                        // add in bytes used by each Bitmap
                        return previous + map_entry.second.getSizeInBytes(portable);
                    });
        }

        static const Bitmap64 frozenView(const char *buf) {
            ...
            for (uint64_t lcv = 0; lcv < map_size; lcv++) {
                ...
                // read map value Bitmap
                const Bitmap read = Bitmap::frozenView(buf, len);
        ...
        // by concatenating one or more Bitmap::write output buffers with the
        ...
        // result, each concatenated output of Bitmap::writeFrozen is preceeded by
         ...
         */
        Bitmap64 operator&(const Bitmap64 &o) const {
            return Bitmap64(*this) &= o;
        }
        ...
        void setCopyOnWrite(bool val) {
            ...
            std::for_each(roarings.begin(), roarings.end(),
                          [=](std::pair<const uint32_t, Bitmap> &map_entry) {
         ...
         * pointer).
         */
        static Bitmap64 fastunion(size_t n, const Bitmap64 **inputs) {
            ...
            // For a given Bitmap64, we look at its underlying 'roarings'
            ...
            while (!pq.empty()) {
                ...
                result.roarings.insert(result.roarings.end(),
                                       std::make_pair(group_key, Bitmap(inner_result)));
            }
            return result;
        }

        friend class Bitmap64SetBitForwardIterator;

        friend class Bitmap64SetBitBiDirectionalIterator;

        typedef Bitmap64SetBitForwardIterator const_iterator;
        typedef Bitmap64SetBitBiDirectionalIterator const_bidirectional_iterator;
    ...
    private:
        typedef std::map<uint32_t, Bitmap> roarings_t;
        ...
        void emplaceOrInsert(const uint32_t key, const Bitmap &value) {
        ...
        void emplaceOrInsert(const uint32_t key, Bitmap &&value) {
#if defined(__GLIBCXX__) && __GLIBCXX__ < 20130322
         ...
         */
        Bitmap &lookupOrCreateInner(uint32_t key) {
         ...
         * refers to a Bitmap bitmap rather being an empty slot. Inserts empty
         * Bitmap bitmaps if necessary. The interval must be valid and non-empty.
        ...
        roarings_t::iterator ensureRangePopulated(uint32_t start_high,
            ...
            for (uint64_t slot = start_high; slot <= end_high; ++slot) {
                ...
                if (next_populated_iter != roarings.end() &&
                    ...
                    // Make a fresh entry {key = 'slot', value = Bitmap()}, insert
 ...
 */
    ...
    public:
        ...
        typedef Bitmap64SetBitForwardIterator type_of_iterator;

        /**
         * Provides the location of the set bit.
         */
        value_type operator*() const {
            return Bitmap64::uniteBytes(map_iter->first, i.current_value);
        }
        ...
        type_of_iterator operator++(int) {  // i++, must return orig. value
            Bitmap64SetBitForwardIterator orig(*this);
        ...
        bool move(const value_type &x) {
            map_iter = p.lower_bound(Bitmap64::highBytes(x));
            if (map_iter != p.cend()) {
                roaring_init_iterator(&map_iter->second.roaring, &i);
                if (map_iter->first == Bitmap64::highBytes(x)) {
                    if (roaring_move_uint32_iterator_equalorlarger(&i, Bitmap64::lowBytes(x)))
                        return true;
                    map_iter++;
                    if (map_iter == map_end) return false;
                    roaring_init_iterator(&map_iter->second.roaring, &i);
        ...
        bool operator==(const Bitmap64SetBitForwardIterator &o) const {
        ...
        Bitmap64SetBitForwardIterator &operator=(const Bitmap64SetBitForwardIterator &r) {
            map_iter = r.map_iter;
            map_end = r.map_end;
            i = r.i;
            return *this;
        ...
        Bitmap64SetBitForwardIterator(const Bitmap64SetBitForwardIterator &r)
                : p(r.p),
                  map_iter(r.map_iter),
                  map_end(r.map_end),
                  i(r.i) {}

        Bitmap64SetBitForwardIterator(const Bitmap64 &parent,
    ...
    protected:
        const std::map<uint32_t, Bitmap> &p;
        std::map<uint32_t, Bitmap>::const_iterator map_iter{}; // The empty constructor silences warnings from pedantic static analyzers.
        std::map<uint32_t, Bitmap>::const_iterator map_end{}; // The empty constructor silences warnings from pedantic static analyzers.
        roaring::api::roaring_uint32_iterator_t i{}; // The empty constructor silences warnings from pedantic static analyzers.
    };

    class Bitmap64SetBitBiDirectionalIterator final : public Bitmap64SetBitForwardIterator {
    public:
        explicit Bitmap64SetBitBiDirectionalIterator(const Bitmap64 &parent,
                                                     bool exhausted = false)
                : Bitmap64SetBitForwardIterator(parent, exhausted), map_begin(parent.roarings.cbegin()) {}

        Bitmap64SetBitBiDirectionalIterator &operator=(const Bitmap64SetBitForwardIterator &r) {
            *(Bitmap64SetBitForwardIterator *) this = r;
            return *this;
        }

        Bitmap64SetBitBiDirectionalIterator &operator--() { //  --i, must return dec.value
            if (map_iter == map_end) {
                --map_iter;
        ...
        Bitmap64SetBitBiDirectionalIterator operator--(int) {  // i--, must return orig. value
            Bitmap64SetBitBiDirectionalIterator orig(*this);
            if (map_iter == map_end) {
                --map_iter;
                roaring_init_iterator_last(&map_iter->second.roaring, &i);
                return orig;
    ...
    protected:
        std::map<uint32_t, Bitmap>::const_iterator map_begin;
    };

    inline Bitmap64SetBitForwardIterator Bitmap64::begin() const {
        return Bitmap64SetBitForwardIterator(*this);
    }

    inline Bitmap64SetBitForwardIterator Bitmap64::end() const {
        return Bitmap64SetBitForwardIterator(*this, true);
    }
