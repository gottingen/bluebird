...
/*! \file nn_dict_dynamic.h
    \brief nn_dict_dynamic.h contains a class for a dynamic bit vector which also supports the prev and next operations
...
//! A class for a dynamic bit vector which also supports the prev and next operations
    ...
    public:
        ...
        public:
            ...
            operator bool() const {
                uint64_t node = m_pbv->m_tree[(m_pbv->m_v_begin_leaves + (m_idx >> 6)) -
                                              m_pbv->m_offset[m_pbv->m_depth]];
