...
/*! \file suffix_array_helper.h
    \brief suffix_array_helper.h contains some helper classes for CSTs
	\author Simon Gog
*/
...
//! A helper class for the \f$\Psi\f$ function for (compressed) suffix arrays which provide also the inverse suffix array values (like bluebird::csa_bitcompressed).
    template<typename t_csa, bool t_direction>
...
//! A wrapper class for the \f$\Psi\f$ and LF function for (compressed) suffix arrays that are based on a wavelet tree (like bluebird::csa_wt).
    template<typename t_csa, bool t_direction>
