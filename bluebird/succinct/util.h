...
/*! \file util.h
    \brief util.h contains some helper methods for int_vector and other stuff like demangle class names.
    \author Simon Gog
*/
...
//! Demangle the class name of typeid(...).name() and remove the "bluebird::"-prefix, "unsigned int",...
...
//! Transforms the demangled class name of an object to a hash value.
        ...
        uint64_t hashvalue_of_classname(const T &) {
            ...
            return str_hash(bluebird::util::demangle2(typeid(T).name()));
        }
