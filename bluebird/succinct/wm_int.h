...
/*! \file wm_int.h
    \brief wm_int.h contains a specialized class for a wavelet tree for
 ...
 */
    ...
    public:
        ...
        wm_int(int_vector_buffer<int_width> &buf, size_type size,
            ...
            bluebird::remove(zero_buf_file_name);
            tree_out_buf.close();
        ...
        const {
            ...
            if (v.level == m_max_level) {
                for (size_type j = 1; j <= bluebird::size(r) and report; ++j) {
                    size_type i = j;
                    size_type c = v.sym;
                    for (uint32_t k = m_max_level; k > 0; --k) {
