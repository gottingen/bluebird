...
/*! \file cst_iterators.h
    \brief cst_iterators.h contains iterator classes for traversing (compressed) suffix arrays.
	\author Simon Gog
*/
...
//! A forward iterator for a bottom up traversal of a suffix tree
    ...
    public:
        ...
        //! Constructor
        cst_bottom_up_const_forward_iterator(const Cst *cst, const value_type node, bool valid = true) : m_valid(
                valid) {
            m_cst = cst;
            m_v = node;
            if (m_cst == nullptr)
