...
/*! \file select_support.h
    \brief select_support.h contains classes that support a bluebird::bit_vector with constant time select information.
	\author Simon Gog
*/
 ...
 * This group contains data structures which support an bluebird::bit_vector with the select method.
 */
...
namespace bluebird {
//! The base class of classes supporting select queries for a bluebird::bit_vector in constant time.
/*! Abstract base class for classes supporting select queries.
 */
    class select_support {
    protected:
        const int_vector<1> *m_v; //!< Pointer to the select supported bluebird::bit_vector.
    public:
    ...
    struct select_support_trait<00, 2> {
        ...
        static size_type arg_cnt(const bit_vector &v) {
            ...
            if (v.bit_size() & 0x3F) {   // if bit_size is not a multiple of 64, add count of the last word
                result += rank_support_trait<00, 2>::args_in_the_word((*data) | bits::lo_unset[v.bit_size() & 0x3F],
                                                                      carry);
            }
            return result;
        }
    ...
    struct select_support_trait<11, 2> {
        ...
        static size_type arg_cnt(const bit_vector &v) {
            ...
            if (v.bit_size() & 0x3F) {   // if bit_size is not a multiple of 64, add count of the last word
                result += rank_support_trait<11, 2>::args_in_the_word((*data) & bits::lo_set[v.bit_size() & 0x3F],
                                                                      carry);
            }
            return result;
        }
