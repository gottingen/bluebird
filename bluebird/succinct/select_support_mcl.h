...
/*! \file select_support_mcl.hpp
    \brief select_support_mcl.hpp contains classes that support a bluebird::bit_vector with constant time select information.
    \author Simon Gog
*/
            ...
            if (select_support_trait<t_b, t_pat_len>::found_arg(i, *v)) {
                ...
                if (arg_cnt % SUPER_BLOCK_SIZE == 0 or arg_cnt == m_arg_cnt) { //
                    ...
                    if (pos_diff > m_logn4) { // longblock
                        ...
                        m_longsuperblock[sb_cnt] = int_vector<0>(SUPER_BLOCK_SIZE, 0, bits::hi(
                                arg_position[(arg_cnt - 1) % SUPER_BLOCK_SIZE]) + 1);
...
//    size_type sb = (m_arg_cnt+63+SUPER_BLOCK_SIZE-1)/SUPER_BLOCK_SIZE; // number of superblocks, add 63 as the last block could contain 63 uninitialized bits
        ...
        for (size_type i = 0, cnt_old = 0, cnt_new = 0, last_k64_sum = 1; i < v->capacity(); i += 64, ++data) {
            ...
            if (cnt_new >= last_k64_sum) {
                arg_position[last_k64 - 1] = i + select_support_trait<t_b, t_pat_len>::ith_arg_pos_in_the_word(*data,
                                                                                                               last_k64_sum -
                                                                                                               cnt_old,
                                                                                                               carry_new);
                last_k64 += 64;
                ...
                if (last_k64 == SUPER_BLOCK_SIZE + 1) {
                    ...
                    if (pos_diff > m_logn4) { // long block
                        ...
                        for (size_type j = arg_position[0], k = 0;
                             k < SUPER_BLOCK_SIZE and j <= pos_of_last_arg_in_the_block; ++j)
    ...
    inline auto select_support_mcl<t_b, t_pat_len>::select(size_type i) const -> size_type {
        ...
        if (m_longsuperblock != nullptr and !m_longsuperblock[sb_idx].empty()) {
            ...
            if ((offset & 0x3F) == 0) {
                ...
                if (args >= i) {
                    return (word_pos << 6) +
                           select_support_trait<t_b, t_pat_len>::ith_arg_pos_in_the_first_word(*data, i, word_off,
                                                                                               carry);
                }
                word_pos += 1;
    ...
    template<uint8_t t_b, uint8_t t_pat_len>
    auto select_support_mcl<t_b, t_pat_len>::serialize(std::ostream &out, structure_tree_node *v,
        ...
        if (m_arg_cnt) { // if there exists 1-bits to be supported
            ...
            structure_tree_node *child_long = structure_tree::add_child(child, "longsuperblock",
                                                                        util::class_name(m_longsuperblock));
            structure_tree::add_size(child_long, written_bytes_long);
            structure_tree_node *child_mini = structure_tree::add_child(child, "minisuperblock",
                                                                        util::class_name(m_miniblock));
