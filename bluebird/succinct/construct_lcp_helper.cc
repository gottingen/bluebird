...
#include "bluebird/succinct/construct_lcp_helper.h"
...
namespace bluebird {
     ...
     */
    void
    insert_lcp_values(int_vector<> &partial_lcp, bit_vector &index_done, std::string lcp_file, uint64_t max_lcp_value,
        ...
        out_buf.close();
        bluebird::rename(tmp_lcp_file, lcp_file);
    }

    buffered_char_queue::buffered_char_queue() : m_widx(0), m_ridx(0), m_sync(true), m_disk_buffered_blocks(0),
                                                 m_c('?'), m_rb(0), m_wb(0) {}
