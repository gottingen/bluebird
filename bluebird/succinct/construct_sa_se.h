...
namespace bluebird {
    ...
    void _construct_sa_se(int_vector_type &text, std::string filename_sa, uint64_t sigma, uint64_t recursion) {
        ...
        int_vector_buffer<> right(tmp_file(filename_sa, "_right" + util::to_string(recursion)), std::ios::out,
                                  buffersize, nsize);
        size_t right_pointer = 0;
        int_vector_buffer<> left(tmp_file(filename_sa, "_left" + util::to_string(recursion)), std::ios::out, buffersize,
                                 nsize);
        ...
        util::_set_zero_bits(text_rec);
        {
            if (recursion == 0 and n / 2 * text_rec.width() > 8 * n) {
                ...
                std::string filename_text_rec_part2 = tmp_file(filename_sa,
                                                               "_text_rec_part2" + util::to_string(recursion));
                size_t pos = 0;
        ...
        if (text_rec.size() > order + 1) {
            if (recursion == 0) {
                ...
                _construct_sa_IS(text_rec, text_rec, filename_sa_rec, number_of_lms_strings, number_of_lms_strings,
                                 order + 1, recursion + 1);
        ...
        if (isa_rec.size() > 0) {
            ...
            bluebird::remove(filename_lms_pos_b);
            ...
            for (size_t i = number_of_lms_strings - 1; i < number_of_lms_strings; --i) {
                ...
                size_t val = lms_select_support.select(
                        i + 1); //TODO test alternative without select support: look for 1 in lms_pos_b (backwards)
                tmp_left[idx] = val;
            }
