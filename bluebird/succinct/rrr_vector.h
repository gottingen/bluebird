...
/*! \file rrr_vector.h
   \brief rrr_vector.h contains the bluebird::rrr_vector class, and
...
#include "bluebird/succinct/rrr_helper.h" // for binomial helper class
 ...
 *    \sa bluebird::rrr_vector for a specialized version for block_size=15
 */
    ...
    public:
        ...
        */
        rrr_vector(const bit_vector &bv) {
            ...
            bt_array.resize(
                    (m_size + t_bs) / ((size_type) t_bs)); // blocks for the bt_array + a dummy block at the end,
            // if m_size%t_bs == 0
            ...
            m_rank = int_vector<>((bt_array.size() + t_k - 1) / t_k + ((m_size % (t_k * t_bs)) > 0), 0,
                                  bits::hi(sum_rank) + 1);
            //                                                ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#include "bluebird/succinct/rrr_vector_15.h" // include specialization
