...
/*! \file io.h
    \brief io.h contains some methods for reading/writing sdsl structures.
    \author Simon Gog
*/
...
namespace bluebird {
    ...
    serialize_vector(const std::vector<T> &, std::ostream &,
                     bluebird::structure_tree_node *v = nullptr, std::string= "");
...
// Writes primitive-typed variable t to stream out
    template<class T>
    size_t
    write_member(const T &t, std::ostream &out, bluebird::structure_tree_node *v = nullptr, std::string name = "") {
        bluebird::structure_tree_node *child = bluebird::structure_tree::add_child(v, name, util::class_name(t));
        out.write((char *) &t, sizeof(t));
        size_t written_bytes = sizeof(t);
        bluebird::structure_tree::add_size(child, written_bytes);
        return written_bytes;
    }

// Specialization for std::string
    template<>
    size_t write_member<std::string>(const std::string &t, std::ostream &out, bluebird::structure_tree_node *v,
                                     std::string name);
...
// TODO: Remove ENDIAN dependency.
    ...
    bool
    load_vector_from_file(t_int_vec &v, const std::string &file, uint8_t num_bytes = 1, uint8_t max_int_width = 64) {
...
//! Store an int_vector as plain int_type array to disk
    ...
    template<class T>
    size_t serialize_empty_object(std::ostream &, structure_tree_node *v = nullptr, std::string name = "",
                                  const T *t = nullptr) {
 ...
 */
    template<class T>
    uint64_t
    serialize_vector(const std::vector<T> &vec, std::ostream &out, bluebird::structure_tree_node *v, std::string name) {
        if (vec.size() > 0) {
            bluebird::structure_tree_node *child = bluebird::structure_tree::add_child(v, name, "std::vector<" +
                                                                                                util::class_name(
                                                                                                        vec[0]) + ">");
            size_t written_bytes = 0;
 ...
 */
    ...
    void write_structure(const X &x, std::ostream &out) {
        ...
        if (st_node.get()->children.size() > 0) {
            for (const auto &child: st_node.get()->children) {
                bluebird::write_structure_tree<F>(child.second.get(), out);
            }
        }
    }
    ...
    void write_structure(std::ostream &out, Xs... xs) {
        ...
        bluebird::write_structure_tree<F>(st_node.get(), out);
    }
...
//==================== Template functions ====================
        ...
        if (hash_value != util::hashvalue_of_classname(v)) {
            if (util::verbose) {
                std::cerr << "File `" << file << "` is not an instance of the class `"
                          << bluebird::util::demangle2(typeid(T).name()) << "`" << std::endl;
            }
            return false;
        }
        return load_from_file(v, file);
    ...
    inline typename std::enable_if<
            ...
            std::is_same<typename t_iv::index_category, lcp_tag>::value, std::ostream &>::type
