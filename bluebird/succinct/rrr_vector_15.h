...
/*! \file rrr_vector.h
   \brief rrr_vector.h contains a specialisation of the bluebird::rrr_vector class,
...
#include "util.h"
#include "rrr_helper.h" // for binomial helper class
#include "rrr_vector.h"
#include "iterators.h"
#include <vector>
    ...
    public:
        ...
        */
        rrr_vector(const bit_vector &bv) {
            ...
            m_rank = int_vector<>((bt_array.size() + t_k - 1) / t_k + ((m_size % (t_k * block_size)) > 0), 0,
                                  bits::hi(sum_rank) + 1);
            //                                                                                    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        */
        const size_type rank(size_type i) const {
            ...
            return rank_support_rrr_trait<t_b>::adjust_rank(rank +
                                                            bits::cnt(((uint64_t) (bi_type::nr_to_bin(last_bt, btnr))) &
                                                                      bits::lo_set[off]), i);
        }
