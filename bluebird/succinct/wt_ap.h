        ...
        wt_ap(int_vector_buffer<int_width> &buf, size_type size) : m_size(size) {
            if (buf.size() < m_size) {
                throw std::logic_error(
                        "n=" + util::to_string(buf.size()) + " < " + util::to_string(m_size) + "=m_size");
                return;
            }
            ...
            value_type class_size = 1;
            {
                ...
                for (value_type i = m_singleton_class_cnt; i < m_class_cnt; ++i) {
                    ...
                    temp_file_offset_buffers.emplace_back(
                            temp_file_offset,
                            int_vector_buffer<wt_int_width>(temp_file_offset, std::ios::out, 1024 * 1024,
                                                            bits::hi(offset) + 1));
                }
                char_freq.clear();
            ...
            std::string temp_file_class = buf.filename()
                                          ...
                                          + "_" + util::to_string(util::id());
            int_vector_buffer<wt_byte_width> class_buffer(temp_file_class, std::ios::out, 1024 * 1024,
            ...
            bluebird::remove(temp_file_class);
            {
                ...
                for (value_type i = 0; i < m_class_cnt - m_singleton_class_cnt; ++i) {
                    ...
                    bluebird::remove(temp_file_offset_buffer.first);
                }
            }
        }
