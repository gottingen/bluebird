...
/*! \file wt_int.h
    \brief wt_int.h contains a specialized class for a wavelet tree of a
...
#include "rank_support_v.h"
#include "select_support_mcl.h"
#include "wt_helper.h"
#include "util.h"
...
namespace bluebird {
    ...
    private:
        ...
        void _interval_symbols(size_type i, size_type j, size_type &k,
            ...
            if ((j - i) - (ones_before_j - ones_before_i) > 0) {
                ...
                _interval_symbols(new_i, new_j, k, cs, rank_c_i, rank_c_j, level + 1, path << 1, new_node_size,
                                  new_offset);
            }
            ...
            if ((ones_before_j - ones_before_i) > 0) {
                ...
                _interval_symbols(new_i, new_j, k, cs, rank_c_i, rank_c_j, level + 1, (path << 1) | 1, new_node_size,
                                  new_offset);
            }
        }
    ...
    public:
        ...
        wt_int(int_vector_buffer<int_width> &buf, size_type size,
            ...
            bluebird::remove(tree_out_buf_file_name);
        ...
        range_search_2d(size_type lb, size_type rb, value_type vlb, value_type vrb,
            ...
            _range_search_2d(lb, rb, vlb, vrb, 0, 0, m_size, offsets, ones_before_os, 0, point_vec, report,
                             cnt_answers);
        ...
        const {
            ...
            if (vlb < mid and mid) {
                ...
                if (nrb)
                    _range_search_2d(nlb, nrb - 1, vlb, std::min(vrb, mid - 1), level + 1, ilb,
                                     zeros_before_end - zeros_before_o, offsets, ones_before_os, path << 1, point_vec,
                                     report, cnt_answers);
            }
            if (vrb >= mid) {
                ...
                if (nrb)
                    _range_search_2d(nlb, nrb - 1, std::max(mid, vlb), vrb, level + 1, mid,
                                     ones_before_end - ones_before_o, offsets, ones_before_os, (path << 1) + 1,
