...
#include "bluebird/succinct/construct_lcp.h"
...
//                    std::cerr<<"n="<<n<<" "<<" plcp.size()="<<plcp.size();
        ...
        int_vector_buffer<> lcp_out_buf(cache_file_name(conf::KEY_LCP, config), std::ios::out, buffer_size,
        ...
        // phase 1: calculate lcp_sml;  memory consumption: 2n bytes (lcp_sml=n bytes, text=n bytes)
        {
            ...
            alphabet[sigma] = '\0';
            {
                ...
                for (size_type i = 1, sai, cur_c_idx = 1, cur_c_cnt = cnt_c[alphabet[1] + 1]; i < n; ++i, --cur_c_cnt) {
                    ...
                    size_type l = 0;
                    if (i >=
                        cnt_cc[cur_c]) { // if the current lcp entry is not already done   TODO: schleife von i bis cnt_cc[cur_c]
            ...
            if (n > 1000 and
                nn > 5 * (n / 6)) {  // if we would occupy more space than the PHI algorithm => switch to PHI algorithm
                util::clear(lcp_sml);
                construct_lcp_PHI<8>(config);
                return;
            }
...
//			std::cout<<"# begin calculating LF' values"<<std::endl;
            int_vector<> lcp_big(nn, 0, bits::hi(n - 1) +
...
//			std::cout<<"# begin initializing bwt2, shift_bwt2, run2"<<std::endl;
            ...
            bit_vector run2(
                    nn + 1);                    // indicates for each entry i, if i and i-1 are both big LCP values
...
//		std::cout<<"# merge lcp_sml and lcp_big"<<std::endl;
        // phase 3: merge lcp_sml and lcp_big and save to disk
        {
            ...
            int_vector_buffer<8> lcp_sml_buf(cache_file_name("lcp_sml", config), std::ios::in,
                                             buffer_size);        // file buffer containing the small LCP values
            int_vector_buffer<> lcp_buf(cache_file_name(conf::KEY_LCP, config), std::ios::out, buffer_size,
    ...
    void construct_lcp_goPHI(cache_config &config) {
        ...
        // phase 2: calculate lcp_big with PHI algorithm on remaining entries of LCP
        {
            int_vector<> lcp_big(0, 0, bits::hi(n - 1) + 1);//nn, 0, bits::hi(n-1)+1);
            {
                ...
                for (size_type i = 0, ii = 0, l = m + 1, p = 0;
                     i < n and ii < nn; ++i) { // execute compact Phi algorithm
                    if (todo[i]) {
                        if (i > 0 and todo[i - 1])
                            l = l - 1;
                        else
...
// (1) Calculate LCP-Positions-Array: For each lcp_value (in ascending order) all its occurrences (in any order) in the lcp array
        {
            ...
            int_vector_buffer<> lcp_positions_buf(tmp_lcp_file, std::ios::out, buffer_size,
                                                  int_width); // Create buffer for positions of LCP entries
            size_type idx_out_buf = 0;
            bit_vector index_done(n + 1,
                                  0);                 // Bitvector which is true, if corresponding LCP value was already calculated
...
// (2) Insert LCP entires into LCP array
        {
            ...
            std::string lcp_file = cache_file_name(conf::KEY_LCP, config);
            int_vector_buffer<> lcp_array(lcp_file, std::ios::out, number_of_values * int_width / 8,
                                          int_width); // Create Output Buffer
            number_of_values = lcp_array.buffersize() * 8 / int_width;

            for (size_type position_begin = 0, position_end = number_of_values; position_begin < n and
                                                                                number_of_values >
                                                                                0; position_begin = position_end, position_end += number_of_values) {
#ifdef STUDY_INFORMATIONS
