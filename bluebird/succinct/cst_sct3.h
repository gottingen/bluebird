...
#include "bluebird/succinct/iterators.h"
#include "bluebird/succinct/lcp.h"
#include "bluebird/succinct/bp_support.h"
#include "bluebird/succinct/csa_wt.h" // for std initialization of cst_sct3
#include "bluebird/succinct/cst_iterators.h"
#include "bluebird/succinct/rank_support.h"
#include "bluebird/succinct/select_support.h"
#include "bluebird/succinct/util.h"
#include "bluebird/succinct/sdsl_concepts.h"
#include "bluebird/succinct/construct.h"
#include "bluebird/succinct/suffix_tree_helper.h"
#include "bluebird/succinct/suffix_tree_algorithm.h"
#include <iostream>
#include <algorithm>
#include <cassert>
    ...
    public:
        ...
        void swap(cst_sct3 &cst) {
            if (this != &cst) {
                ...
                util::swap_support(m_first_child_select, cst.m_first_child_select, &m_first_child,
                                   &(cst.m_first_child));
...
//Procedure:(1) Determine, if v has a right sibling.
            if (v.cipos <
                v.jp1pos) { // LCP[i] > LCP[j+1] => v has the same right border as parent(v) => no right sibling
                return root();
            }
//          (2)    There exists a right sibling, LCP[j+1] >= LCP[i] and j>i
            ...
            if (last_child) {
                ...
                return node_type(v.j + 1, new_j, v.jp1pos, m_bp_support.find_close(v.jp1pos),
...
// == template functions ==
    ...
    template<class t_csa, class t_lcp, class t_bp_support, class t_bv, class t_rank, class t_sel>
    auto cst_sct3<t_csa, t_lcp, t_bp_support, t_bv, t_rank, t_sel>::serialize(std::ostream &out, structure_tree_node *v,
    ...
    cst_sct3<t_csa, t_lcp, t_bp_support, t_bv, t_rank, t_sel>::operator=(const cst_sct3 &cst) {
        if (this != &cst) {
            copy(cst);
        }
        return *this;
    ...
    template<class t_csa, class t_lcp, class t_bp_support, class t_bv, class t_rank, class t_sel>
    cst_sct3<t_csa, t_lcp, t_bp_support, t_bv, t_rank, t_sel> &
    cst_sct3<t_csa, t_lcp, t_bp_support, t_bv, t_rank, t_sel>::operator=(cst_sct3 &&cst) {
        if (this != &cst) {
    ...
    struct bp_interval {
        ...
        //! Constructor
        bp_interval(t_int i = 0, t_int j = 0, t_int ipos = 0, t_int cipos = 0, t_int jp1pos = 0) : i(i), j(j),
                                                                                                   ipos(ipos),
                                                                                                   cipos(cipos),
                                                                                                   jp1pos(jp1pos) {};
