 ...
 */
    ...
    _symbol_lte(const t_wt &wt, typename t_wt::value_type c) {
        if (((1ULL) << (wt.max_level)) <= c) {
            ...
            c = bluebird::bits::lo_set[wt.max_level];
        }
        auto node = wt.root();
 ...
 */
    ...
    restricted_unique_range_values(const t_wt &wt,
        ...
        std::stack<std::tuple<typename t_wt::node_type, bluebird::range_type, uint64_t, uint64_t>> stack;
        bluebird::range_type initial_range = {x_i, x_j};
