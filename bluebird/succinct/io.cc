...
#include "bluebird/succinct/io.h"
#include "bluebird/succinct/sfstream.h"
...
namespace bluebird {
    ...
    template<>
    size_t
    write_member<std::string>(const std::string &t, std::ostream &out, structure_tree_node *v, std::string name) {
    ...
    std::string tmp_file(const cache_config &config, std::string name_part) {
        return config.dir + "/" + util::to_string(util::pid()) + "_" + util::to_string(util::id()) + name_part +
               ".sdsl";
    }
